...
def build_czech_one_pager(output_path):
    ...
    mod_data = [
        ...
        [Paragraph("<b>1. ARGOS-AI</b> (Vlajková loď)", table_cell), Paragraph("Kat. 4: Edge AI", table_cell), Paragraph("Detekce impaktních kráterů v INT8 Micro-CNN + laserová fúze PALT pro metrické rozměry", table_cell), Paragraph("2.39 s WCET | 142 kB Scratchpad<br/><i>(+1.04 MB Camera Frame)</i>", table_cell)],
        [Paragraph("<b>2. DEEP-WAVE</b>", table_cell), Paragraph("Kat. 2: Komprese", table_cell), Paragraph("Reverzibilní vlnková komprese CDF 5/3 šetřící 82.2 % přenosového pásma na Zemi", table_cell), Paragraph("2.39 s WCET | 38 kB Scratchpad<br/><i>(Tile-based streaming)</i>", table_cell)],
        [Paragraph("<b>3. AURA-GNC</b>", table_cell), Paragraph("Kat. 1: Autonomie & GNC", table_cell), Paragraph("Shadow-Mode 3D triangulace tělesa z kráterů, gravitační inverze GM a validace vůči ESOC", table_cell), Paragraph("3.80 s WCET | 96 kB Scratchpad<br/><i>(9stavový EKF filtr)</i>", table_cell)],
        [Paragraph("<b>4. AEGIS-FDIR</b>", table_cell), Paragraph("Kat. 5: Odolnost & FDIR", table_cell), Paragraph("In-flight realizace výzkumu ESTEC (HERA-IoD): Isolation Forest pro detekci anomálií telemetrie", table_cell), Paragraph("0.12 s WCET | 18 kB Scratchpad<br/><i>(Quantized trees)</i>", table_cell)],
    ...
    story.append(Paragraph(
        ...
        "Uvedené paměťové nároky představují statický pracovní scratchpad algoritmů; obrazová data (1,04 MB / snímek) jsou zpracovávána streamově v DMA bufferech. "
        "Kód byl plně otestován na oficiální sadě <b>2 400+ reálných kalibračních snímků kamery AFC</b> v emulátoru QEMU pro SPARC V8 (LEON3). "
...
def build_english_one_pager(output_path):
    ...
    mod_data = [
        ...
        [Paragraph("<b>1. ARGOS-AI</b> (Flagship)", table_cell), Paragraph("Cat. 4: Edge AI", table_cell), Paragraph("Impact crater detection in INT8 Micro-CNN + PALT laser altimeter metric sizing in meters", table_cell), Paragraph("2.39 s WCET | 142 kB Scratchpad<br/><i>(+1.04 MB Camera Frame)</i>", table_cell)],
        [Paragraph("<b>2. DEEP-WAVE</b>", table_cell), Paragraph("Cat. 2: Compression", table_cell), Paragraph("Reversible CDF 5/3 integer wavelet compression achieving -82.2% downlink bandwidth reduction", table_cell), Paragraph("2.39 s WCET | 38 kB Scratchpad<br/><i>(Tile-based streaming)</i>", table_cell)],
        [Paragraph("<b>3. AURA-GNC</b>", table_cell), Paragraph("Cat. 1: Autonomy & GNC", table_cell), Paragraph("Shadow-Mode 3D landmark mesh building from craters, GM gravity inversion & ESOC validation", table_cell), Paragraph("3.80 s WCET | 96 kB Scratchpad<br/><i>(9-state EKF filter)</i>", table_cell)],
        [Paragraph("<b>4. AEGIS-FDIR</b>", table_cell), Paragraph("Cat. 5: Resilience & FDIR", table_cell), Paragraph("Operationalizing ESTEC HERA-IoD research: Quantized Isolation Forest telemetry anomaly detector", table_cell), Paragraph("0.12 s WCET | 18 kB Scratchpad<br/><i>(Quantized trees)</i>", table_cell)],
    ...
    story.append(Paragraph(
        ...
        "Stated memory budgets represent static algorithm scratchpads; camera frame buffers (1.04 MB / image) are processed via streaming DMA. "
    ...
    if os.path.exists(gdrive_dir):
        for src, name in [
            ...
            try:
                ...
                print(f"[NOTE] Could not overwrite {name} on Google Drive: {e}")
